...
import type { PredictionText } from '@/app/(features)/prediction/_types'
...
const PredictionDetail = async ({ id }: PredictionDetailProps) => {
	...
	const jstTime = new Date(predictionInfo.endTime * 1000).toLocaleString(
		'ja-JP',
		{
			timeZone: 'Asia/Tokyo',
		},
	)
	const utcTime = new Date(predictionInfo.endTime * 1000).toLocaleString(
		'en-US',
	)
	...
	return (
		<div className='container mx-auto max-w-xl pb-12'>
			...
			<Typography variant='h4'>
				Rate: x{(100 + predictionInfo.rate) / 100}
			</Typography>
			<br />

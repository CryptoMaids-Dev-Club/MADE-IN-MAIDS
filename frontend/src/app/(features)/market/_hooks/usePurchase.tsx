...
export const usePurchase = ({ item }: usePurchaseProps) => {
	...
	const { allowance, refetch } = useAllowance(
		address ?? `0x${''}`,
		maidsMarketAddress[NETWORK.id],
	)
	const { approve, isPending: isLoadingApprove } = useApprove(
		maidsTokenAddress[NETWORK.id],
	)
	...
	const buttonMessage = useCallback(() => {
		...
		if (balance < BigInt(item.price) * BigInt(amount))
			return 'Insufficient $MAIDS'
